...
"""Class for bitplusd node under test"""
...
# 'bitplus' command, it may internally insert more args, which must be
...
BITPLUS_PID_FILENAME_DEFAULT = "bitplusd.pid"
...
class TestNode():
    """A class for representing a bitplusd node under test.
    ...
    def __init__(
        ...
        *,
        ...
        self.bitplusconf = self.datadir_path / "bitplus.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the bitplus.conf file.
        # This means that starting a bitplusd using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    PRIV_KEYS = [
            # address , privkey
            AddressKeyPair('TDv1D3WV2gFK87ucb8bDG7KGjNh5HCPZPi', 'Vh9EqbUrpkENJX8DSBYbv8kNKygSgyDC2PoYczRPKLSKk5SYp6o8'),
            AddressKeyPair('TMyMSQpgJR4QL6z6u5zz2dLWudi1x5Fobw', 'VgHsDYkbW5CqaexqFzFRgkqBzw3MYCJURkVKx4U8XntfWpBzFwky'),
            AddressKeyPair('THG2uNG2VASsFWK4DmpZZpkwKtC5Qjkyoo', 'VfBgpDAr7aAJGnS1z2zWwJnEyn8q5wsqZ1nV1L7GrEZF3WJSRdKB'),
            AddressKeyPair('TKmAXb9sZaic2wUR6dAN4WXEWw4AwjKkA1', 'VhEK29YJYaYub7tVfWxcvcEphEbmRrqnh1BWwcvBbZMTN6nhFobM'),
            AddressKeyPair('TMzqK7ifxnfe9AQFeTRKew51G7z8G7n66i', 'VbYBtUkEgWeVhRCsQ6THpjCGWfAij1ckgZd1Uyoh2XK6P8zkvoiM'),
            AddressKeyPair('TXK3cV5ggPDY3DLkuTRGBzcE7yhvMQrc47', 'VbukUEp4HNgzUAjuqT1khHL72jZetx7Ezx3iSANqzi7qgyM9kwTa'),
            AddressKeyPair('TUTA6xihDMjdJLaFnh511NrU5DpYooAazF', 'VbgouujZ744rpLmGy7UipaenFn1x6V2mbMmz7ScrJpEWCLbcgmNH'),
            AddressKeyPair('TQECAaedwev5rwXnCsrhk9pxr4aVX4mVqs', 'Vdrm7tx7ZjfasZmAP8DEdfcU1tDo8qMUgpZW8MpWezZdxZMmuHhQ'),
            AddressKeyPair('TJwNHGjShSytwkBWywn4FvLjxBSLrrAasS', 'Ve8XagKNzkheEEs297P22U7My4QS5HnSgi4NfKLL12pjqCnTzRTQ'),
            AddressKeyPair('TKWutNvN5yacv6MjcKpvTLhEvPZnFa4rjn', 'VZQ5LA5dEjkcMN4pt2LbbcTRNnEMpdw1BGTXc1x6kdVwYNx1Ky3H'),
            AddressKeyPair('TJhQzE2GT2YM4QdsNWZyEDtaoQ6Vt8mQAc', 'VeSq1h6dtgPuY7iUW8sVEWepdfrHzryuXjSUvwer8xaNoLZCWVT4'),
            AddressKeyPair('TUstqQr6zyPmWPZX5aKNMCS3Jf7eBF8N4A', 'Vb3RCoYwJfK78UQ25GEaEhKPqGY5K2v4zzRaewo9YNJ2giYwSEPV'),
    ]
    ...
    def __del__(self):
        # Ensure that we don't leave any bitplusd processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then bitplusd would bind P2P ports on
        ...
        # bitplus.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time bitplusd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by bitplusd, and
        ...
        self.running = True
        self.log.debug("bitplusd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the bitplusd process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'bitplusd exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; bitplusd may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to bitplusd after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; bitplusd is still starting
    ...
    def wait_until_stopped(self, *, timeout=BITPLUSD_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into bitplusd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to bitplusd
        expected_msg: regex that stderr should match when bitplusd fails

        Will raise if bitplusd starts without an error.
        Will raise if an expected_msg is provided and it does not match bitplusd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'bitplusd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'bitplusd should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to bitplus-cli for an individual node"""
    def __init__(self, binaries, datadir, rpc_timeout):
        ...
        self.input = None
        self.log = logging.getLogger('TestFramework.bitpluscli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bitplus-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run bitplus-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running bitplus-cli {}".format(p_args[2:]))
