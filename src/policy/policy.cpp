...
bool IsStandardAssetLockingScript(const CScript& script)
{
    CScript::const_iterator pc{script.begin()};
    opcodetype opcode;
    std::vector<unsigned char> data;

    if (!script.GetOp(pc, opcode) || opcode != OP_SHA256) return false;
    if (!script.GetOp(pc, opcode, data) || opcode > OP_PUSHDATA4 || data.size() != uint256::size()) return false;
    if (!script.GetOp(pc, opcode) || opcode != OP_EQUAL) return false;
...
CAmount GetDustThreshold(const CTxOut& txout, const CFeeRate& dustRelayFeeIn)
{
    ...
    // See discussion in https://github.com/bitplus/bitplus/pull/22779 for details.
...
std::vector<uint32_t> GetDust(const CTransaction& tx, CFeeRate dust_relay_rate)
{
    ...
    for (uint32_t i{0}; i < tx.vout.size(); ++i) {
        if (bitplus::assets::DecodeAssetCommitmentScript(tx.vout[i].scriptPubKey).has_value()) continue;
...
bool IsStandard(const CScript& scriptPubKey, TxoutType& whichType)
{
    ...
    if (whichType == TxoutType::NONSTANDARD) {
        const std::optional<CScript> asset_locking_script{bitplus::assets::DecodeAssetCommitmentLockingScript(scriptPubKey)};
        if (!asset_locking_script.has_value()) return false;
        if (asset_locking_script->empty() || asset_locking_script->IsUnspendable()) return false;
        if (bitplus::assets::DecodeAssetCommitmentLockingScript(*asset_locking_script).has_value()) return false;
        if (IsStandardAssetLockingScript(*asset_locking_script)) return true;

        TxoutType asset_locking_type;
        return IsStandard(*asset_locking_script, asset_locking_type);
