...
#include <qt/bitplusaddressvalidator.h>
...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "btp1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq2jku9f";
        break;
    case ChainType::SIGNET:
    case ChainType::TESTNET:
    case ChainType::TESTNET4:
        addr = "tbtp1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqa6qnlg";
        break;
    case ChainType::REGTEST:
        addr = "btprt1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqsr2427";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitplus address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitplusAddressEntryValidator(parent));
    widget->setCheckValidator(new BitplusAddressCheckValidator(parent));
}
...
bool parseBitplusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitplus: URI
    if(!uri.isValid() || uri.scheme() != QString("bitplus"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitplusUnits::parse(BitplusUnit::BTP, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitplusURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitplusURI(uriInstance, out);
}

QString formatBitplusURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitplus:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitplusUnits::format(BitplusUnit::BTP, info.amount, false, BitplusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitplusConf()
{
    ...
    /* Open bitplus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitplus.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitplus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Bitplus (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Bitplus*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "bitplus.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitplus-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a bitplus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Bitplus\n";
        else
            optionFile << strprintf("Name=Bitplus (%s)\n", ChainTypeToString(chain));
