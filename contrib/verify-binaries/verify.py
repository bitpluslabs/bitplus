...
"""Script for verifying Bitplus Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitpluscore.org and bitplus.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/bitplus-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://bitpluscore.org"
HOST2 = "https://bitplus.org"
VERSIONPREFIX = "bitplus-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Bitplus community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"bitplus_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by bitpluscore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the bitplus release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes bitplus.org lags behind bitpluscore.org.)')
    )
