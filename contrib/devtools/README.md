...
Generates a bitplus.conf file in `share/examples/` by parsing the output from `bitplusd --help`. This script is run during the
release process to include a bitplus.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-bitplus-conf.sh
```
