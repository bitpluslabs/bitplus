Bitplus Core version 0.19.0.1 is now available from:

  <https://bitpluscore.org/bin/bitplus-core-0.19.0.1/>
...
Bitplus Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Bitplus Core on unsupported systems.
...
to open the Bitplus Core .dmg. This is due to new signing requirements
imposed by Apple, which the Bitplus Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitplus/bitplus/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Bitplus Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Bitplus
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitplusd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Bitplus Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Bitplus Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Bitplus Core. (#14054)

- To eliminate well-known denial-of-service vectors in Bitplus Core,
  ...
  This prevents Bitplus Core from sending the BIP111 NODE_BLOOM service
  ...
  of Bitplus Core and nodes that have manually enabled BIP37 support,
...
- By default, Bitplus Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitplus-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Bitplus Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Bitplus Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Bitplus Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Bitplus address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Bitplus Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Bitplus AEAD (jonasschnelli)
...
- #15639 bitplus-wallet tool: Drop libbitplus_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in bitplusd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitplus-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitplusd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitplus_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitplus to auto generated project files (sipsorcery)
...
- #15773 Add BitplusTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitplus output (practicalswift)
...
- #15896 feature_filelock, interface_bitplus_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitplus Core (luke-jr)
...
- #16277 Suppress output in test_bitplus for expected errors (gertjaap)
...
- #16489 log: harmonize bitplusd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitplusVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitplus.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBBITPLUS_SERVER (Bushstar)
...
- #16080 build/doc: update bitplus_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitplus.conf (fanquake)
...
- #16621 add default bitplus.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitplus-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitplus.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitplusd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitplus/bitplus/).
