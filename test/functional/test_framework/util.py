...
def assert_fee_amount(fee, tx_size, feerate_BTP_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_BTP_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s BTP too low! (Should be %s BTP)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTP_kvB)
    if fee > high_fee:
...
class Binaries:
    """Helper class to provide information about bitplus binaries
    ...
    def __init__(self, paths, bin_dir, *, use_valgrind=False):
        ...
        self.valgrind_cmd = [
            ...
            "--trace-children=yes",  # Needed for 'bitplus' wrapper
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke bitplusd"
        return self._argv("node", self.paths.bitplusd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke bitplus-cli"
        # Add -nonamed because "bitplus rpc" enables -named by default, but bitplus-cli doesn't
        return self._argv("rpc", self.paths.bitpluscli) + ["-nonamed"]

    def bench_argv(self):
        "Return argv array that should be used to invoke bench_bitplus"
        return self._argv("bench", self.paths.bitplus_bench)

    def tx_argv(self):
        "Return argv array that should be used to invoke bitplus-tx"
        return self._argv("tx", self.paths.bitplustx)

    def util_argv(self):
        "Return argv array that should be used to invoke bitplus-util"
        return self._argv("util", self.paths.bitplusutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke bitplus-wallet"
        return self._argv("wallet", self.paths.bitpluswallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke bitplus-chainstate"
        return self._argv("chainstate", self.paths.bitpluschainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command.

        It either uses the bitplus wrapper executable (if BITPLUS_CMD is set or
        need_ipc is True), or the direct binary path (bitplusd, etc). When
        ...
        (previous releases). Also, valgrind will only apply to the bitplus
        wrapper executable directly, not to the commands that `bitplus` calls.
        """
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.bitplus_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the bitplus
            # wrapper binary and append -m so it calls multiprocess binaries.
            bitplus_cmd = self.paths.bitplus_cmd or [self.paths.bitplus_bin]
            return self.valgrind_cmd + bitplus_cmd + (["-m"] if need_ipc else []) + [command]
        else:
...
def get_binary_paths(config):
    ...
    binaries = {
        "bitplus": "BITPLUS_BIN",
        "bitplusd": "BITPLUSD",
        "bench_bitplus": "BITPLUS_BENCH",
        "bitplus-cli": "BITPLUSCLI",
        "bitplus-util": "BITPLUSUTIL",
        "bitplus-tx": "BITPLUSTX",
        "bitplus-chainstate": "BITPLUSCHAINSTATE",
        "bitplus-wallet": "BITPLUSWALLET",
    }
    # Set paths to bitplus core binaries allowing overrides with environment
    # variables.
    ...
    # BITPLUS_CMD environment variable can be specified to invoke bitplus
    # wrapper binary instead of other executables.
    paths.bitplus_cmd = shlex.split(os.getenv("BITPLUS_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_btp_kvb):
    """Calculate the fee in BTP given a feerate is BTP/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btp_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  BTP
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `BitplusTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/bitplus/bitplus/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitplus.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Bitplus"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Bitplus"
        else:
            datadir = temp_dir / ".bitplus"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitplus.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitplus.conf")):
        with open(os.path.join(datadir, "bitplus.conf"), 'r') as f:
            for line in f:
