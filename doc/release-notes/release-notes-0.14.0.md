Bitplus Core version 0.14.0 is now available from:

  <https://bitplus.org/bin/bitplus-core-0.14.0/>
...
Bitplus Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/bitplus/bitplus/blob/master/doc/zmq.md) for
...
When Bitplus Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `bitplus-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`bitplus-cli` has been updated to support this by parsing `name=value` arguments
...
The Alert System was [disabled and deprecated](https://bitplus.org/en/alert/2016-11-01-alert-retirement) in Bitplus Core 0.12.1 and removed in 0.13.0.
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Bitplus Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/bitplus/bitplus/pull/8753)
...
- #8722 `bb843ad` bitplus-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run bitplus-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to bitplus-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to bitplus-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and bitplus-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_bitplus_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change bitplus address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` bitplusd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow bitplus-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` bitplus-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to bitplus-tx (jnewbery)
