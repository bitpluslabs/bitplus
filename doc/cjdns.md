# CJDNS support in Bitplus Core

It is possible to run Bitplus Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitplus Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitplus network and individual nodes.
...
## Run Bitplus Core with CJDNS

Once you are connected to the CJDNS network, the following Bitplus Core
...
When enabled, this option tells Bitplus Core that it is running in an
...
IPv6 local network. This helps Bitplus Core perform better address management:
...
CJDNS support was added to Bitplus Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitplus-cli -addrinfo` to see the
...
## CJDNS-related information in Bitplus Core

There are several ways to see your CJDNS address in Bitplus Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitplus-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitplus-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitplus-cli help getnodeaddresses` for details.
