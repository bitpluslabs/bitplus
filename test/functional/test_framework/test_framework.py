...
from .address import create_deterministic_address_btprt1_p2tr_op_true
...
TMPDIR_PREFIX = "bitplus_func_test_"
...
class BitplusTestMetaClass(type):
    """Metaclass for BitplusTestFramework.

    Ensures that any attempt to register a subclass of `BitplusTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitplusTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitplusTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitplusTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitplusTestFramework(metaclass=BitplusTestMetaClass):
    """Base class for a bitplus test script.

    Individual bitplus test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitplusds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitplus-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BitplusRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitplusd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitplusd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bitplusd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as bitplusd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            gen_addresses = [k.address for k in TestNode.PRIV_KEYS][:3] + [create_deterministic_address_btprt1_p2tr_op_true()[0]]
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitplus.conf
    ...
    def skip_if_no_bitplusd_tracepoints(self):
        """Skip the running test if bitplusd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitplusd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitplusd_zmq(self):
        """Skip the running test if bitplusd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitplusd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitplus-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitplus-wallet has not been compiled")

    def skip_if_no_bitplus_tx(self):
        """Skip the running test if bitplus-tx has not been compiled."""
        if not self.is_bitplus_tx_compiled():
            raise SkipTest("bitplus-tx has not been compiled")

    def skip_if_no_bitplus_util(self):
        """Skip the running test if bitplus-util has not been compiled."""
        if not self.is_bitplus_util_compiled():
            raise SkipTest("bitplus-util has not been compiled")

    def skip_if_no_bitplus_chainstate(self):
        """Skip the running test if bitplus-chainstate has not been compiled."""
        if not self.is_bitplus_chainstate_compiled():
            raise SkipTest("bitplus-chainstate has not been compiled")

    def skip_if_no_bitplus_bench(self):
        """Skip the running test if bench_bitplus has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_bitplus has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitplus-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitplus-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_bitplus was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether bitplus-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitplus-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitplus_tx_compiled(self):
        """Checks whether bitplus-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITPLUS_TX")

    def is_bitplus_util_compiled(self):
        """Checks whether bitplus-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITPLUS_UTIL")

    def is_bitplus_chainstate_compiled(self):
        """Checks whether bitplus-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_BITPLUS_CHAINSTATE")
