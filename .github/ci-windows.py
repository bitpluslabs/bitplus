...
def check_manifests(ci_type):
    ...
    manifest_path = release_dir / "bitplusd.manifest"
    cmd_bitplusd_manifest = [
        "mt.exe",
        "-nologo",
        f"-inputresource:{release_dir / 'bitplusd.exe'}",
        f"-out:{manifest_path}",
    ]
    run(cmd_bitplusd_manifest)
...
def prepare_tests(ci_type):
    ...
    if ci_type == "standard":
        ...
        clone_cmd = [
            ...
            "clone",
            "--depth=1",
            "https://github.com/bitplus-core/qa-assets",
            repo_dir,
        ]
        run(clone_cmd)
...
def run_tests(ci_type):
    ...
    if ci_type == "standard":
        ...
        test_envs = {
            "BITPLUS_BIN": "bitplus.exe",
            "BITPLUSD": "bitplusd.exe",
            "BITPLUSCLI": "bitplus-cli.exe",
            "BITPLUS_BENCH": "bench_bitplus.exe",
            "BITPLUSTX": "bitplus-tx.exe",
            "BITPLUSUTIL": "bitplus-util.exe",
            "BITPLUSWALLET": "bitplus-wallet.exe",
            "BITPLUSCHAINSTATE": "bitplus-chainstate.exe",
        }
        ...
        os.environ["BITPLUSFUZZ"] = str(release_bin / "fuzz.exe")
        fuzz_cmd = [
            sys.executable,
