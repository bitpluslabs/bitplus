...
"""Test bitplus-chainstate tool functionality

Test basic block processing via bitplus-chainstate tool, including detecting
duplicates and malformed input.

Test that bitplus-chainstate can load a datadir initialized with an assumeutxo
...
class BitplusChainstateTest(BitplusTestFramework):
    def skip_test_if_missing_module(self):
        self.skip_if_no_bitplus_chainstate()
    ...
    def basic_test(self):
        ...
        self.log.info(f"Test bitplus-chainstate {self.get_binaries().chainstate_argv()} with datadir: {datadir}")
    ...
    def assumeutxo_test(self, dump_output_path):
        ...
        datadir = n1.chain_path
        n1.stop_node()
        self.log.info(f"Test bitplus-chainstate {self.get_binaries().chainstate_argv()} with an assumeutxo datadir: {datadir}")
...
if __name__ == "__main__":
    BitplusChainstateTest(__file__).main()
