...
Bitplus Core version v30.0 is now available from:

  <https://bitpluscore.org/bin/bitplus-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Bitplus-Qt` (on macOS)
...
Bitplus Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Bitplus
...
- A new `bitplus` command line tool has been added to make features more discoverable
  and convenient to use. The `bitplus` tool just calls other executables and does not
  implement any functionality on its own. Specifically `bitplus node` is a synonym for
  `bitplusd`, `bitplus gui` is a synonym for `bitplus-qt`, and `bitplus rpc` is a synonym
  for `bitplus-cli -named`. Other commands and options can be listed with `bitplus help`.
  The new `bitplus` command is an alternative to calling other commands directly, but it
...
- The new `bitplus` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `bitplus -m node -ipcbind=unix` it will
  ...
  (`bitplus-node` instead of `bitplusd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/bitplus/bitplus/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/bitplus/bitplus/blob/master/doc/multiprocess.md)),
...
- The `test_bitplus` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `bitplus` command
  as `bitplus test`. The `libexec/` directory also contains new `bitplus-node` and
  `bitplus-gui` binaries which support IPC features and are called through the
  `bitplus` tool. In source builds only, `test_bitplus-qt`, `bench_bitplus`, and
  `bitplus-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `bitplus` command. See `bitplus help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Bitplus Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Bitplus Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the bitplus-wallet tool,
...
[Transifex](https://explore.transifex.com/bitplus/bitplus/).
