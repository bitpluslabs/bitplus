 ...
 *
 * See https://bitplustalk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror) {
    ...
    // https://bitplus.stackexchange.com/a/12556:
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKOUTPUTVERIFY:
                {
                    // OP_CHECKOUTPUTVERIFY is a Bitplus institutional-contract opcode.
                    if (sigversion != SigVersion::TAPSCRIPT || !(flags & SCRIPT_VERIFY_INSTITUTIONAL_CONTRACTS)) {
                        return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
                    }

                    // (scriptPubKeyHash amount output_index -- )
                    if (stack.size() < 3) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

                    const valtype& script_pubkey_hash = stacktop(-3);
                    const CScriptNum amount(stacktop(-2), fRequireMinimal, 8);
                    const CScriptNum output_index(stacktop(-1), fRequireMinimal, 4);

                    if (!checker.CheckOutput(output_index, amount, script_pubkey_hash)) {
                        return set_error(serror, SCRIPT_ERR_CHECKOUTPUTVERIFY);
                    }
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckOutput(const CScriptNum& nOutput, const CScriptNum& nAmount, std::span<const unsigned char> scriptPubKeyHash) const
{
    if (scriptPubKeyHash.size() != uint256::size()) return false;
    if (nOutput < 0 || nAmount < 0) return false;

    const int64_t output_index{nOutput.GetInt64()};
    if (output_index >= static_cast<int64_t>(txTo->vout.size())) return false;

    const CTxOut& output{txTo->vout[static_cast<size_t>(output_index)]};
    if (output.nValue != nAmount.GetInt64()) return false;

    const uint256 expected_hash{(HashWriter{} << output.scriptPubKey).GetSHA256()};
    return std::equal(expected_hash.begin(), expected_hash.end(), scriptPubKeyHash.begin());
}
...
static bool ExecuteWitnessScript(const std::span<const valtype>& stack_span, const CScript& exec_script, script_verify_flags flags, SigVersion sigversion, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    if (sigversion == SigVersion::TAPSCRIPT) {
        ...
        while (pc < exec_script.end()) {
            ...
            if (opcode == OP_CHECKOUTPUTVERIFY && !(flags & SCRIPT_VERIFY_INSTITUTIONAL_CONTRACTS)) {
...
#define FLAG_NAME(flag) {std::string(#flag), SCRIPT_VERIFY_##flag}
    static const std::map<std::string, script_verify_flag_name> g_names_to_enum{
        ...
        FLAG_NAME(INSTITUTIONAL_CONTRACTS),
    };
#undef FLAG_NAME
