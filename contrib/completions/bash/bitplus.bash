# bash programmable completion for bitplus(1) wrapper
...
_bitplus() {
    ...
    local bitplus subcmd offset

    # save and use original argument to invoke bitplus for help
    ...
    case "$subcmd" in
        gui|node)
            _bitplus_wrap bitplusd "$offset"
            return 0
            ;;
        rpc)
            _bitplus_wrap bitplus-cli "$offset"
            return 0
            ;;
        tx)
            _bitplus_wrap bitplus-tx "$offset"
            return 0
            ;;
    esac
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                options=$($bitplus help 2>&1 | awk '{ for(i=1;i<=NF;i++) if ($i~/^--/) { sub(/=.*/, "=",$i); print $i } }' )
            fi
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$($bitplus help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
