...
std::string BitplusAssetTypeToString(bitplus::assets::AssetCommitmentType type)
{
    switch (type) {
...
UniValue BitplusAssetToUniv(const bitplus::assets::AssetCommitment& commitment)
{
    ...
    asset.pushKV("metadata_hash", commitment.metadata_hash.ToString());
    asset.pushKV("member_hash", commitment.member_hash.ToString());
    asset.pushKV("commitment_hash", bitplus::assets::HashAssetCommitment(commitment).ToString());
    return asset;
}

UniValue BitplusMetadataToUniv(const bitplus::assets::AssetMetadataCommitment& commitment)
{
    ...
    metadata.pushKV("commitment_hash", bitplus::assets::HashAssetMetadataCommitment(commitment).ToString());
    return metadata;
}

UniValue BitplusWhitelistToUniv(const bitplus::assets::AssetWhitelistCommitment& commitment)
{
    ...
    whitelist.pushKV("flags", static_cast<int64_t>(commitment.flags));
    whitelist.pushKV("commitment_hash", bitplus::assets::HashAssetWhitelistCommitment(commitment).ToString());
    return whitelist;
}

UniValue BitplusWhitelistProofToUniv(const bitplus::assets::AssetWhitelistProofCommitment& commitment)
{
    ...
    proof.pushKV("members_root", bitplus::assets::ComputeWhitelistMembersRoot(commitment).ToString());
    proof.pushKV("commitment_hash", bitplus::assets::HashAssetWhitelistProofCommitment(commitment).ToString());
    return proof;
}
...
void ScriptToUniv(const CScript& script, UniValue& out, bool include_hex, bool include_address, const SigningProvider* provider)
{
    ...
    if (const std::optional<bitplus::assets::AssetCommitment> asset{bitplus::assets::DecodeAssetCommitmentScript(script)}) {
        UniValue asset_obj{BitplusAssetToUniv(*asset)};
        if (const std::optional<CScript> locking_script{bitplus::assets::DecodeAssetCommitmentLockingScript(script)}) {
            UniValue locking_script_obj{UniValue::VOBJ};
            ScriptToUniv(*locking_script, locking_script_obj, include_hex, include_address, provider);
            asset_obj.pushKV("locking_script", std::move(locking_script_obj));
        }
        out.pushKV("bitplus_asset", std::move(asset_obj));
    } else if (const std::optional<bitplus::assets::AssetMetadataCommitment> metadata{bitplus::assets::DecodeAssetMetadataCommitmentScript(script)}) {
        out.pushKV("bitplus_asset_metadata", BitplusMetadataToUniv(*metadata));
    } else if (const std::optional<bitplus::assets::AssetWhitelistCommitment> whitelist{bitplus::assets::DecodeAssetWhitelistCommitmentScript(script)}) {
        out.pushKV("bitplus_asset_whitelist", BitplusWhitelistToUniv(*whitelist));
    } else if (const std::optional<bitplus::assets::AssetWhitelistProofCommitment> proof{bitplus::assets::DecodeAssetWhitelistProofCommitmentScript(script)}) {
        out.pushKV("bitplus_asset_whitelist_proof", BitplusWhitelistProofToUniv(*proof));
    }
}
