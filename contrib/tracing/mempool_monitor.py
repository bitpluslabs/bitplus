...
""" Example logging Bitplus Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into bitplusd with pid {pid}")
    bitplusd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitplusd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitplusd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitplusd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitplusd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitplusd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of bitplusd>")
        exit(1)
