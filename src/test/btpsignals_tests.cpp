...
BOOST_AUTO_TEST_CASE(disconnects)
{
    ...
    {
        btpsignals::scoped_connection scope(conn0);
    }
    BOOST_CHECK(!conn0.connected());
    BOOST_CHECK(sig0.empty());
    sig0(val);
...
{
    btpsignals::signal<MoveOnlyData(int)> sig0;
...
{
    btpsignals::signal<bool()> sig0;
    decltype(sig0)::result_type ret;
    ret = sig0();
    BOOST_CHECK(!ret);
    {
        btpsignals::scoped_connection conn0 = sig0.connect(ReturnTrue);
        ret = sig0();
    ...
    BOOST_CHECK(!ret);
    {
        btpsignals::scoped_connection conn1 = sig0.connect(ReturnTrue);
        btpsignals::scoped_connection conn0 = sig0.connect(ReturnFalse);
        ret = sig0();
...
BOOST_AUTO_TEST_CASE(thread_safety)
{
    ...
    std::thread extra_increment_injector([&conn0, &sig0, &val] {
        ...
        std::vector<btpsignals::scoped_connection> extra_conns;
...
{
    btpsignals::connection conn0, conn1, conn2;
    btpsignals::signal<void()> sig0;
