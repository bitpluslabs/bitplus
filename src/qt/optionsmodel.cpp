...
/** Call node.updateRwSetting() with Bitplus 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bitplus 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitplus/bitplus/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitplus 22.x will not try to read these.
...
//! Convert enabled/size values to bitplus -prune setting.
...
//! Get pruning enabled value to show in GUI from bitplus -prune setting.
...
//! Get pruning size value to show in GUI from bitplus -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitplusUnit")) {
        settings.setValue("DisplayBitplusUnit", QVariant::fromValue(BitplusUnit::BTP));
    }
    QVariant unit = settings.value("DisplayBitplusUnit");
    if (unit.canConvert<BitplusUnit>()) {
        m_display_bitplus_unit = unit.value<BitplusUnit>();
    } else {
        m_display_bitplus_unit = BitplusUnit::BTP;
        settings.setValue("DisplayBitplusUnit", QVariant::fromValue(m_display_bitplus_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitplus.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitplus_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitplusUnit>() == m_display_bitplus_unit) return;
    m_display_bitplus_unit = new_unit.value<BitplusUnit>();
    QSettings settings;
    settings.setValue("DisplayBitplusUnit", QVariant::fromValue(m_display_bitplus_unit));
    Q_EMIT displayUnitChanged(m_display_bitplus_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitplus/bitplus/pull/8273
    ...
    // (https://github.com/bitplus-core/gui/issues/567).
