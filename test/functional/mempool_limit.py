...
class MempoolLimitTest(BitplusTestFramework):
    ...
    def test_mid_package_eviction_success(self):
        ...
        fees_btp_per_kvb = [entry["fees"]["base"] / (Decimal(entry["vsize"]) / 1000) for entry in mempool_entries]
        mempool_entry_minrate = min(fees_btp_per_kvb)
        ...
        for eviction in eviction_set:
            ...
            for txid, entry in zip(mempool_txids, mempool_entries):
                if txid == eviction:
                    evicted_feerate_btp_per_kvb = entry["fees"]["modified"] / (Decimal(entry["vsize"]) / 1000)
                    assert_greater_than(evicted_feerate_btp_per_kvb, max_parent_feerate)
    ...
    def run_test(self):
        ...
        worst_feerate_btpvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_btpvb = min(worst_feerate_btpvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_feerate = (worst_feerate_btpvb * 1000) - Decimal("0.0000001")  # 0.01 sats/vbyte below worst feerate
        ...
        assert_greater_than(worst_feerate_btpvb, package_fee / package_vsize)
