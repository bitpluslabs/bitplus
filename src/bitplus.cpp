...
Options:
  -m, --multiprocess     Run multiprocess binaries bitplus-node, bitplus-gui.
  -M, --monolithic       Run monolithic binaries bitplusd, bitplus-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'bitplus-qt [ARGS]' or 'bitplus-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'bitplusd [ARGS]' or 'bitplus-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'bitplus-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'bitplus-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'bitplus-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_bitplus [ARGS]'.
  chainstate [ARGS] Run bitplus kernel chainstate util, equivalent to running 'bitplus-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_bitplus [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_bitplus-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "bitplus-gui" : "bitplus-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "bitplus-node" : "bitplusd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("bitplus-cli");
            // Since "bitplus rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("bitplus-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("bitplus-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_bitplus");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("bitplus-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_bitplus");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_bitplus-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("bitplus-util");
        } else {
...
//! Execute the specified bitplusd, bitplus-qt or other command line in `args`
...
//!             be a relative path to a bitplusd, bitplus-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/bitplus ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/bitplus/bitplus/pull/31375#discussion_r1861814807)
