...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitplus-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitplus-cli', help='bitplus-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitplus address to which the faucet should send')
...
parser.add_argument('bitplus_cli_args', nargs='*', help='Arguments to pass on to bitplus-cli (default: -signet)')

args = parser.parse_args()

if args.bitplus_cli_args == []:
    args.bitplus_cli_args = ['-signet']


def bitplus_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitplus_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitplus_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitplus_cli(['getnewaddress', 'faucet', 'bech32'])
