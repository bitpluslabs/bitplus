...
Tracing (USDT) support for Bitplus Core on Linux using. For more information on
USDT support in Bitplus Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `bitplusd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bitplusd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Bitplus Core USDT documentation] on how to list available tracepoints in your
`bitplusd` binary.

[Bitplus Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof bitplusd)
```
...
Bitplus P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof bitplusd)
```
...
In a different terminal, starting Bitplus Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/bitplusd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof bitplusd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof bitplusd)
```
