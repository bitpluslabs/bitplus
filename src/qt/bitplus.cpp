...
#include <qt/bitplus.h>

#include <btpsignals.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<BitplusUnit>("BitplusUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitplus_de.qm (shortcut "de" needs to be defined in bitplus.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitplus_de_DE.qm (shortcut "de_DE" needs to be defined in bitplus.qrc)
...
static const char* qt_argv = "bitplus-qt";

BitplusApplication::BitplusApplication()
...
void BitplusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitplusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitplusGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitplusApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitplusApplication::createOptionsModel(bool resetSettings)
{
...
void BitplusApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitplusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitplusGUI::quitRequested, this, &BitplusApplication::requestShutdown);
...
void BitplusApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitplusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitplusApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitplusApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitplusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitplusApplication::handleRunawayException);
    connect(this, &BitplusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitplusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitplusApplication::parameterSetup()
{
...
void BitplusApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitplusApplication::requestInitialize()
{
...
void BitplusApplication::requestShutdown()
{
...
void BitplusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // bitplus: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitplusGUI::handlePaymentRequest);
        connect(window, &BitplusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitplusApplication::handleRunawayException(const QString &message)
{
...
void BitplusApplication::handleNonFatalException(const QString& message)
{
...
WId BitplusApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitplusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitplusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    btpsignals::scoped_connection handler_message_box = ::uiInterface.ThreadSafeMessageBox_connect(noui_ThreadSafeMessageBox);
    btpsignals::scoped_connection handler_question = ::uiInterface.ThreadSafeQuestion_connect(noui_ThreadSafeQuestion);
    btpsignals::scoped_connection handler_init_message = ::uiInterface.InitMessage_connect(noui_InitMessage);

    // Do not refer to data directory yet, this can be overridden by Intro::pickDataDirectory

    /// 1. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(bitplus);
    Q_INIT_RESOURCE(bitplus_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BITPLUS_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitplus-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitplus-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitplus.conf, determine network, switch to network specific
    ...
    // bitplus: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
