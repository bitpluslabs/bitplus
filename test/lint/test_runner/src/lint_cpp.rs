...
pub fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/bitplus-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <bitplus-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the bitplus-build-config.h header. However, they are not
...
even though bitplus-build-config.h indicates that a faster feature is available and should be used.
...
#include <bitplus-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the bitplus-build-config.h header. However, they are including
...
pub fn lint_std_filesystem() -> LintResult {
    let found = git()
        .args([
            ...
            ":(exclude)src/bitplus-chainstate.cpp",
        ])
        .status()
        .expect("command error")
        .success();
