...
const char * const BITPLUS_CONF_FILENAME = "bitplus.conf";
const char * const BITPLUS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitplus-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITPLUS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Bitplus
    //   new: C:\Users\Username\AppData\Local\Bitplus
    // macOS: ~/Library/Application Support/Bitplus
    // Unix-like: ~/.bitplus
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bitplus";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bitplus";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Bitplus";
#else
    // Unix-like
    return pathRet / ".bitplus";
#endif
#endif
}
