...
Bitplus Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_bitplus`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_bitplus

After compiling bitplus-core, the benchmarks can be run with:

    build/bin/bench_bitplus
...
To monitor Bitplus Core performance more in depth (like reindex or IBD): https://github.com/bitplus-dev-tools/benchcoin
