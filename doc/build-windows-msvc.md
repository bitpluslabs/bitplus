...
This guide describes how to build bitplusd, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
Clone the Bitplus Core repository to a directory. All build scripts and commands will run from this directory.
...
The Bitplus Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Bitplus Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to Bitplus Core.
...
can occur if your local Bitplus Core repository path contains spaces, you can override the vcpkg install directory
...
To improve the build process performance, one might add the Bitplus repository directory to the Microsoft Defender Antivirus exclusions.
