...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1BTP/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> bitplus-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> bitplus-cli -named " + methodname;
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: application/json' http://127.0.0.1:9456/\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: application/json' http://127.0.0.1:9456/\n";
}
    ...
    return
         {
             ...
             {RPCResult::Type::OBJ, "bitplus_asset", /*optional=*/true, "Decoded Bitplus native asset carrier commitment.", {
                 ...
                 {RPCResult::Type::STR_HEX, "metadata_hash", "Asset metadata commitment hash."},
                 {RPCResult::Type::STR_HEX, "member_hash", "Settlement member hash."},
                 {RPCResult::Type::STR_HEX, "commitment_hash", "Asset commitment hash."},
                 {RPCResult::Type::OBJ, "locking_script", /*optional=*/true, "Decoded inner asset locking script.", {
                     {RPCResult::Type::STR, "asm", "Disassembly of the inner locking script"},
                     {RPCResult::Type::STR, "desc", "Inferred descriptor for the inner locking script"},
                     {RPCResult::Type::STR_HEX, "hex", "The raw inner locking script bytes, hex-encoded"},
                     {RPCResult::Type::STR, "address", /*optional=*/true, "The Bitplus address (only if a well-defined address exists)"},
                     {RPCResult::Type::STR, "type", "The inner locking script type"},
                 }},
             }},
             {RPCResult::Type::OBJ, "bitplus_asset_metadata", /*optional=*/true, "Decoded Bitplus asset metadata commitment.", {
                 ...
                 {RPCResult::Type::STR_HEX, "commitment_hash", "Metadata commitment hash."},
             }},
             {RPCResult::Type::OBJ, "bitplus_asset_whitelist", /*optional=*/true, "Decoded Bitplus whitelist rules commitment.", {
                 ...
                 {RPCResult::Type::NUM, "flags", "Whitelist rule flags."},
                 {RPCResult::Type::STR_HEX, "commitment_hash", "Whitelist commitment hash."},
             }},
             {RPCResult::Type::OBJ, "bitplus_asset_whitelist_proof", /*optional=*/true, "Decoded Bitplus whitelist membership proof commitment.", {
                 ...
                 {RPCResult::Type::ARR, "merkle_path", "Merkle sibling path.", {
                     {RPCResult::Type::STR_HEX, "sibling", "A Merkle sibling hash."},
                 }},
                 {RPCResult::Type::STR_HEX, "members_root", "Merkle root computed from this proof."},
                 {RPCResult::Type::STR_HEX, "commitment_hash", "Whitelist proof commitment hash."},
             }},
         };
}
