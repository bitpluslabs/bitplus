...
class InitTest(BitplusTestFramework):
    """
    ...
    def init_stress_test_interrupt(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # bitplusd to perform any shutdown logic.
    ...
    def init_pid_test(self):
        BITPLUS_PID_FILENAME_CUSTOM = "my_fancy_bitplus_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = BITPLUS_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / BITPLUS_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / BITPLUS_PID_FILENAME_CUSTOM
