# bash programmable completion for bitplus-cli(1)
# Copyright (c) 2012-present The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $bitplus-cli for RPC
_bitplus_rpc() {
...
_bitplus_cli() {
    ...
    # save and use original argument to invoke bitplus-cli for -help, help and RPC
    # as bitplus-cli might not be in $PATH
    bitplus_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($bitplus_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_bitplus_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _bitplus_cli bitplus-cli
