...
be used in Bitplus Core with the `-asmap` option.
...
to binary before being used with Bitplus Core.
...
the `bitplus-cli getnodeaddresses` command.
...
```
bitplus-cli getnodeaddresses 0 > addrs.json
```
