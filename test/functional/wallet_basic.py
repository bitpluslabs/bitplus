...
class WalletTest(BitplusTestFramework):
    ...
    def run_test(self):
        ...
        # Sendmany 5 BTP to two addresses with subtracting fee from both addresses
        ...
        fee_rate_sat_vb = 2
        fee_rate_btp_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_btp_kvb = Decimal(fee_rate_btp_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_sat_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_btp_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_btp_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        for target, mode in product([-1, 0], ["btp/kb", "sat/b"]):
