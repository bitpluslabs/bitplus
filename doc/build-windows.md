...
Below are some notes on how to build Bitplus Core for Windows.

The options known to work for building Bitplus Core on Windows are:
...
Note that for WSL the Bitplus Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitplus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitplus`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/bitplus
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/bitplus --strip
```
