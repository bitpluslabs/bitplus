...
static void SetupBitplusTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitplus-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitplus-tx tool is used for creating and modifying bitplus transactions.\n\n"
                "bitplus-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitplus transaction, or with \"-create [commands]\" to create a hex-encoded bitplus transaction.\n"
                "\n"
                "Usage: bitplus-tx [options] <hex-tx> [commands]\n"
                "or:    bitplus-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitplus transaction
