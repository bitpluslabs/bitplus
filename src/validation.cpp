...
struct InstitutionalAssetCheckFailure {
    const char* reason{""};
    std::string debug{};
};
...
static std::optional<InstitutionalAssetCheckFailure> CheckInstitutionalAssetRules(const CTransaction& tx, const CCoinsViewCache& view)
{
    ...
    const std::vector<bitplus::assets::AssetMetadataOutput> metadata_outputs{
        bitplus::assets::ExtractAssetMetadataOutputs(tx)
    };
    const std::vector<bitplus::assets::AssetWhitelistOutput> whitelist_outputs{
        bitplus::assets::ExtractAssetWhitelistOutputs(tx)
    };
    const std::vector<bitplus::assets::AssetWhitelistProofOutput> whitelist_proof_outputs{
        bitplus::assets::ExtractAssetWhitelistProofOutputs(tx)
    };

    const std::optional<bitplus::assets::AssetValidationResult> invalid_output{
        bitplus::assets::FindFirstInvalidAssetOutput(created_asset_outputs)
    };
    if (invalid_output.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_output->reason,
            .debug = strprintf("invalid asset output %u in transaction %s",
                               invalid_output->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<std::vector<bitplus::assets::AssetOutput>> spent_asset_outputs{
        bitplus::assets::ExtractSpentAssetOutputs(tx, view)
    };
    if (!spent_asset_outputs.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = "bad-txns-asset-missing-input",
            .debug = "asset conservation check missing spent output in transaction " + tx.GetHash().ToString(),
        };
    }
    const std::optional<bitplus::assets::AssetValidationResult> invalid_spent_output{
        bitplus::assets::FindFirstInvalidAssetOutput(*spent_asset_outputs)
    };
    if (invalid_spent_output.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_spent_output->reason,
            .debug = strprintf("invalid spent asset output %u in transaction %s",
                               invalid_spent_output->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> invalid_metadata{
        bitplus::assets::FindFirstInvalidAssetMetadataOutput(metadata_outputs)
    };
    if (invalid_metadata.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_metadata->reason,
            .debug = strprintf("invalid asset metadata output %u in transaction %s",
                               invalid_metadata->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> invalid_whitelist{
        bitplus::assets::FindFirstInvalidAssetWhitelistOutput(whitelist_outputs)
    };
    if (invalid_whitelist.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_whitelist->reason,
            .debug = strprintf("invalid asset whitelist output %u in transaction %s",
                               invalid_whitelist->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> invalid_whitelist_proof{
        bitplus::assets::FindFirstInvalidAssetWhitelistProofOutput(whitelist_proof_outputs)
    };
    if (invalid_whitelist_proof.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_whitelist_proof->reason,
            .debug = strprintf("invalid asset whitelist proof output %u in transaction %s",
                               invalid_whitelist_proof->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> unlinked_metadata{
        bitplus::assets::FindFirstUnlinkedAssetMetadata(created_asset_outputs, metadata_outputs)
    };
    if (unlinked_metadata.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = unlinked_metadata->reason,
            .debug = strprintf("missing metadata for asset issuance output %u in transaction %s",
                               unlinked_metadata->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> invalid_issuance_anchor{
        bitplus::assets::FindFirstInvalidAssetIssuanceAnchor(created_asset_outputs, tx)
    };
    if (invalid_issuance_anchor.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = invalid_issuance_anchor->reason,
            .debug = strprintf("invalid issuance anchor for asset output %u in transaction %s",
                               invalid_issuance_anchor->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> unlinked_whitelist{
        bitplus::assets::FindFirstUnlinkedAssetWhitelist(metadata_outputs, whitelist_outputs)
    };
    if (unlinked_whitelist.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = unlinked_whitelist->reason,
            .debug = strprintf("missing whitelist for asset metadata output %u in transaction %s",
                               unlinked_whitelist->output_index, tx.GetHash().ToString()),
        };
    }

    const std::optional<bitplus::assets::AssetValidationResult> unproven_transfer{
        bitplus::assets::FindFirstUnprovenWhitelistedTransfer(created_asset_outputs, metadata_outputs, whitelist_outputs, whitelist_proof_outputs)
    };
    if (unproven_transfer.has_value()) {
        return InstitutionalAssetCheckFailure{
            .reason = unproven_transfer->reason,
            .debug = strprintf("missing or invalid whitelist proof for asset transfer output %u in transaction %s",
                               unproven_transfer->output_index, tx.GetHash().ToString()),
        };
    }

    const std::vector<bitplus::assets::AssetConservationResult> asset_results{
        bitplus::assets::CheckAssetConservation(*spent_asset_outputs, created_asset_outputs)
    };

    const auto invalid_asset{std::ranges::find_if(asset_results, [](const auto& result) {
        return !result.balanced || result.overflow;
    })};
    if (invalid_asset != asset_results.end()) {
        return InstitutionalAssetCheckFailure{
            .reason = "bad-txns-asset-conservation",
            .debug = strprintf("asset conservation failed for asset %s in transaction %s",
                               invalid_asset->asset_id.ToString(), tx.GetHash().ToString()),
        };
    }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_INSTITUTIONAL_CONTRACTS)) {
        const std::optional<InstitutionalAssetCheckFailure> institutional_asset_failure{
            CheckInstitutionalAssetRules(tx, m_view)
        };
        if (institutional_asset_failure.has_value()) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS,
                                 institutional_asset_failure->reason,
                                 institutional_asset_failure->debug);
        }
    }
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_INSTITUTIONAL_CONTRACTS)) {
        scriptVerifyFlags |= SCRIPT_VERIFY_INSTITUTIONAL_CONTRACTS;
    }
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/bitplus/bitplus/issues/22596 for additional information.
...
script_verify_flags GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Enforce Bitplus institutional-contract opcodes.
    if (DeploymentActiveAt(block_index, chainman, Consensus::DEPLOYMENT_INSTITUTIONAL_CONTRACTS)) {
        flags |= SCRIPT_VERIFY_INSTITUTIONAL_CONTRACTS;
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const bool fInstitutionalContracts{DeploymentActiveAt(*pindex, m_chainman, Consensus::DEPLOYMENT_INSTITUTIONAL_CONTRACTS)};
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (fInstitutionalContracts) {
            const std::optional<InstitutionalAssetCheckFailure> institutional_asset_failure{
                CheckInstitutionalAssetRules(tx, view)
            };
            if (institutional_asset_failure.has_value()) {
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              institutional_asset_failure->reason,
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in Bitplus’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/bitplus-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
 ...
 *  grow our in-memory block index. See https://bitpluscore.org/en/2024/07/03/disclose-header-spam.
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // blocks to prevent timewarp attacks (see https://github.com/bitplus/bitplus/pull/15482).
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/bitplus-dev/2019-February/016697.html.  Because CheckBlock() is
