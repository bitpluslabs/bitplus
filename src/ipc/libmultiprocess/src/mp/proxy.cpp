...
thread_local ThreadContext g_thread_context; // NOLINT(bitplus-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking bitplus code so they have to run
    ...
    // functions run, so client objects are fully disconnected before bitplus
    // code in the destructors are run. This way if the bitplus code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In bitplus core, running these callbacks in the right order is
