...
static RPCMethod echo(const std::string& name)
{
    return RPCMethod{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "bitplus-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCMethod{
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new bitplus-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new bitplus-echo executable,
                // and spawn bitplus-echo below instead of bitplus-node. But
                // using bitplus-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("bitplus-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a bitplusd
                // process not a bitplusd-node process, so just create a local
