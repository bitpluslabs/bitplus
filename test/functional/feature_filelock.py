...
"""Check that it's not possible to start a second bitplusd instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import BitplusTestFramework
from test_framework.test_node import (
    BITPLUS_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(BitplusTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second bitplusd instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second bitplusd instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second bitplusd using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second bitplusd instance shutdown
        pid_file = datadir / BITPLUS_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second bitplusd instance using the same wallet")
