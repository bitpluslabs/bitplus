 ...
 */
static const char* BITPLUS_PID_FILENAME = "bitplusd.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", BITPLUS_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", BITPLUS_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-allowignoredconf", strprintf("For backwards compatibility, treat an unused %s file in the datadir as a warning, not an error.", BITPLUS_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-persistmempoolv1",
                   ...
                   ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BITPLUS_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", BITPLUS_CONF_FILENAME, BITPLUS_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-bitplusassetindex", strprintf("Maintain a live Bitplus asset UTXO index for asset scan and stats RPCs (default: %u)", DEFAULT_BITPLUSASSETINDEX), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogWarning("Relative datadir option '%s' specified, which will be interpreted relative to the "
                   "current working directory '%s'. This is fragile, because if bitplus is started in the future "
                   "from a different location, it will be unable to locate the current data files. There could "
                   "also be data loss if bitplus is started while in a temporary directory.",
    ...
    if (args.GetBoolArg("-bitplusassetindex", DEFAULT_BITPLUSASSETINDEX)) {
        g_bitplus_asset_index = std::make_unique<BitplusAssetIndex>(interfaces::MakeChain(node), index_cache_sizes.bitplus_asset_index, false, do_reindex);
        node.indexes.emplace_back(g_bitplus_asset_index.get());
    }
