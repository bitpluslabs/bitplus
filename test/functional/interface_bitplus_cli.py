...
# The block reward of coinbaseoutput.nValue (50) BTP/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 BTP/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"bitplus-cli -h\" for help or \"bitplus-cli listwallets\" to see "
...
class TestBitplusCli(BitplusTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `bitplus-cli getblockchaininfo`")
        ...
        # Prevent bitplus-cli from using existing rpcport in conf
        ...
        assert_raises_process_error(1, "Cookie file credentials were invalid and no rpcpassword was specified.", self.nodes[0].cli(f"-rpccookiefile={self.nodes[0].datadir_path / 'bitplus.conf'}").echo)
        ...
        self.log.info("Test -getinfo and bitplus-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and bitplus-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BTP/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 BTP) to the default wallet.
            ...
            # Test bitplus-cli -generate with -rpcwallet in multiwallet mode.
        ...
        if not self.is_ipc_compiled():
            ...
            self.log.info("Test bitplus-cli -ipcconnect triggers error if not built with IPC support")
            args = [self.binary_paths.bitpluscli, "-ipcconnect=unix", "-getinfo"]
            result = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            assert_equal(result.stdout, "error: bitplus-cli was not built with IPC support\n")
            assert_equal(result.stderr, None)
            assert_equal(result.returncode, 1)


if __name__ == '__main__':
    TestBitplusCli(__file__).main()
