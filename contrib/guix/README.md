# Bootstrappable Bitplus Core Builds

This directory contains the files necessary to perform bootstrappable Bitplus
Core builds.
...
To build Bitplus Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bitplus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitplus-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitplus-core/guix.sigs).
