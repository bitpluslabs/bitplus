...
The headless daemon `bitplusd` has the JSON-RPC API enabled by default, the GUI
`bitplus-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by bitplus-cli when a `-rpcwallet=` parameter is passed in.
...
bitplus-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
bitplus-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
bitplus-cli -named createwallet mywallet load_on_startup=true
```

`bitplus rpc` can also be substituted for `bitplus-cli -named`, and is a newer alternative.

## Versioning

The RPC interface might change from one major version of Bitplus Core to the
...
The RPC interface allows other programs to control Bitplus Core,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure Bitplus Core to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running Bitplus Core can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your Bitplus Core
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use Bitplus Core for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  an RPC interface on the same port as used by Bitplus Core in order to
  trick you into revealing your authentication credentials.  For this
  reason, it is important to only use Bitplus Core for
  security-sensitive operations on a computer whose other programs you
  trust.

- **RPC Credentials Security Boundary:** Any client with valid RPC credentials
  should be treated as having significant control over both the Bitplus Core node
  and the filesystem resources accessible by the `bitplusd` process. RPC commands
  can load wallet files from paths that the `bitplusd` process has permission to
  ...
  not only the Bitplus Core node, but also the machine it is running on. Bitplus Core
...
- **Securing remote network access:** You may optionally allow other
  computers to remotely control Bitplus Core by setting the `rpcallowip`
  ...
  Internet.**  Although Bitplus Core's RPC interface does use
  ...
  `bitplusd -help` for more information about these settings and other
  settings described in this document.

    Related, if you use Bitplus Core inside a Docker container, you may
    need to expose the RPC port to the host system.  The default way to
    do this in Docker also exposes the port to the public Internet.
    Instead, expose it only on the host system's localhost, for example:
    `-p 127.0.0.1:8332:8332`

- **Secure authentication:** By default, when no `rpcpassword` is specified, Bitplus Core generates unique
  login credentials each time it restarts and puts them into a file
  readable only by the user that started Bitplus Core, allowing any of
  that user's RPC clients with read access to the file to login
  automatically.  The file is `.cookie` in the Bitplus Core
  configuration directory, and using these credentials is the preferred
  RPC authentication method.  If you need to generate static login
  credentials for your programs, you can use the script in the
  `share/rpcauth` directory in the Bitplus Core source tree.  As a final
