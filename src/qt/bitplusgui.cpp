...
#include <bitplus-build-config.h> // IWYU pragma: keep

#include <qt/bitplusgui.h>
 ...
 *
 * Ref: https://github.com/bitplus/bitplus/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string BitplusGUI::DEFAULT_UIPLATFORM =
...
BitplusGUI::BitplusGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BitplusGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitplusGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitplusGUI::showModalOverlay);
...
void BitplusGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bitplus address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitplus: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BitplusGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitplusGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitplusGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bitplus addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bitplus addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bitplus Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bitplus Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bitplus: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bitplus command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitplusGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitplusGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitplusGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitplusGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitplusGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitplusGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitplusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitplusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BitplusGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BitplusGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BitplusGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitplusGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitplusGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitplusGUI::showDebugWindow);
}

void BitplusGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BitplusGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitplusGUI::setCurrentWalletBySelectorIndex);
...
void BitplusGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitplusGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitplusGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitplusGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitplusGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitplusGUI::enableHistoryAction(bool privacy)
{
...
void BitplusGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitplusGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitplusGUI::removeWallet);
...
WalletController* BitplusGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitplusGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitplusGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitplusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitplusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitplusGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitplusGUI::incomingTransaction);
    connect(this, &BitplusGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void BitplusGUI::removeWallet(WalletModel* walletModel)
{
...
void BitplusGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitplusGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitplusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitplusGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitplusGUI::createTrayIcon()
{
...
void BitplusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitplusGUI::toggleHidden);
...
void BitplusGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitplusGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitplusGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitplusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitplusGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitplusGUI::openClicked()
{
...
void BitplusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitplusGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitplusGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitplusGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitplusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitplusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitplusGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitplusGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bitplus network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitplusGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitplusGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitplusGUI::updateHeadersSyncProgressLabel()
{
...
void BitplusGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitplusGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitplusGUI::quitRequested);
...
void BitplusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitplusGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &BitplusGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BitplusGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitplusGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitplusGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitplusGUI::show);
                e->ignore();
            }
        }
    }
...
void BitplusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitplusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitplusGUI::incomingTransaction(const QString& date, BitplusUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitplusUnits::formatWithUnit(unit, amount, true));
...
void BitplusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitplusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitplusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitplusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitplusGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitplusGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitplusGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitplusGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BitplusGUI::updateWindowTitle()
{
...
void BitplusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitplusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitplusGUI::detectShutdown()
{
...
void BitplusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitplusGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitplusGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitplusGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitplusGUI::subscribeToCoreSignals()
{
...
void BitplusGUI::unsubscribeFromCoreSignals()
{
...
bool BitplusGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitplusUnit> units = BitplusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitplusUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitplusUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitplusUnit u : BitplusUnits::availableUnits()) {
        menu->addAction(BitplusUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitplusUnit newUnits)
{
    setText(BitplusUnits::longName(newUnits));
}
