...
Bitplus Core version 24.0.1 is now available from:

  <https://bitpluscore.org/bin/bitplus-core-24.0.1/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Bitplus
...
This version of Bitplus Core adds a new `mempoolfullrbf` configuration
...
Some Bitplus services today expect that the first version of an
...
The Bitplus Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Bitplus Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Bitplus Core are
...
[Bitplus Core 0.12.0]: https://bitpluscore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/bitplus/bips/blob/master/bip-0125.mediawiki
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Bitplus Core wallet yet.
  You can find more about Miniscript on the [reference website](https://bitplus.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Bitplus Core wallet, change output
...
[documentation](https://github.com/bitplus/bitplus/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the bitplus GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to bitplusd, instead of being
ignored. (#15936, gui#602)

- Also, the interaction between GUI settings and `bitplus.conf` settings is
simplified. Settings from `bitplus.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`bitplus.conf` values. (#15936)
