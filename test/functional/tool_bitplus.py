...
"""Test the bitplus wrapper tool."""
from test_framework.test_framework import (
    BitplusTestFramework,
    SkipTest,
)
...
class ToolBitplusTest(BitplusTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `bitplus node -version` is
        # run on windows, python doesn't capture output from the child
        # `bitplusd` and `bitplus-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/bitplus/bitplus/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through bitplus wrapper command with specified arguments."""
        # Manually construct the `bitplus node` command, similar to Binaries::node_argv()
        bitplus_cmd = node.binaries.valgrind_cmd + [node.binaries.paths.bitplus_bin]
        node.args = bitplus_cmd + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure bitplus node command invokes bitplusd by default")
        self.test_args([], [], expect_exe="bitplusd")

        self.log.info("Ensure bitplus -M invokes bitplusd")
        self.test_args(["-M"], [], expect_exe="bitplusd")

        self.log.info("Ensure bitplus -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure bitplus -m invokes bitplus-node")
            self.test_args(["-m"], [], expect_exe="bitplus-node")

            self.log.info("Ensure bitplus -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="bitplus-node")

            self.log.info("Ensure bitplus accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="bitplus-node")

            self.log.info("Ensure bitplus recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="bitplus-node")
