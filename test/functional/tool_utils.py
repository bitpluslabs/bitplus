...
class ToolUtils(BitplusTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_bitplus_tx()
        self.skip_if_no_bitplus_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "bitplus-util-test.json") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./bitplus-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./bitplus-tx":
